...
export default function SellForm({
  ...
  const handleSell = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const newQuantity =
      selectedQuantity - (sellQuantity ? Number(sellQuantity) : 0);
    const currentPrice = formData.get("currentPrice");
    const newFunds =
      funds + (currentPrice ? Number(currentPrice) * Number(sellQuantity) : 0);
    ...
    if (newQuantity === 0) {
      try {
        ...
        alert(
          `Successfully sold all shares of ${selectedTicker} and your balance is now ${newFunds}.`
        );
      } catch (error) {
      ...
      try {
        ...
        await addFunds(email, newFunds);
        alert(
          `Successfully sold ${sellQuantity} shares of ${selectedTicker} and still own ${newQuantity} shares and your balance is now ${newFunds}.`
        );
      } catch (error) {
  ...
  const handleSelect = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedStock = stocks.find(
      (stock) => stock.ticker === event.target.value
    );
    if (selectedStock) {
      ...
      <Card className="mt-8">
        <CardContent>
          ...
          <form onSubmit={handleSell}>
            ...
            <div>
              Current Price:{" "}
              {stockPrices[selectedTicker]?.data[0]?.close.toFixed(2) || 0}
            </div>
            <input
              type="hidden"
              name="currentPrice"
              value={stockPrices[selectedTicker]?.data[0]?.close || 0}
            />
            <Input
              type="number"
