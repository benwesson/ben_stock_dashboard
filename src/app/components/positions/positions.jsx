...
export default async function Positions() {
      ...
      try {
        ...
        for (const ticker of tickers) {
          ...
          if (stockData?.data && Array.isArray(stockData.data)) {
            ...
            console.log(
              `${ticker}: ${chartData[ticker].length} closing prices extracted`
            );
          } else {
            chartData[ticker] = [];
  ...
  const currentTotalValue = response.reduce((total, stock) => {
    const latestPrice =
      stockPricesData[stock.ticker]?.data?.[0]?.close || stock.price || 0;
    ...
    <div>
      ...
      >
        {response.map((stock) => {
          ...
          const dayChange = latestOpen
            ? ((latestPrice - latestOpen) / latestOpen) * 100
            : 0;
          ...
          return (
            ...
            >
              ...
              <p>
                Shares: <strong>{stock.quantity}</strong>
              </p>
              <p>
                Total Value:{" "}
                <strong>${(latestPrice * stock.quantity).toFixed(2)}</strong>
              </p>
              ...
              {stockData?.data?.[0] && (
                ...
                >
                  <p>
                    Open: ${latestOpen?.toFixed(2)} | High: $
                    {latestHigh?.toFixed(2)} | Low: ${latestLow?.toFixed(2)}
                  </p>
                  <p>Data Points: {dataPoints}</p>
                  <button>
                    <Link href="/trade">Sell Stock</Link>
                  </button>{" "}
                  <button>
                    <Link href="/trade">Buy More </Link>
                  </button>
                </div>
              )}
            </div>
